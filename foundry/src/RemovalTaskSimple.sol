...
/**
 * @title RemovalTaskSimple
 * @dev Simplified individual bounty/escrow task for data broker removal
 * @notice Represents a single removal request for one broker + subject combination
 * @author Pierce
 */
contract RemovalTaskSimple is ReentrancyGuard {
    ...
    function completeTask() external nonReentrant {
        ...
        // Transfer verifier reward back to factory for distribution
        IERC20(paymentToken).safeTransfer(factory, verifierReward);
    ...
    /**
     * @dev Add authorized verifier (simplified - called by factory)
     */
    function addAuthorizedVerifier(address verifier) external onlyFactory {
        authorizedVerifiers[verifier] = true;
    }
    
    /**
     * @dev Remove authorized verifier (simplified - called by factory)
     */
    function removeAuthorizedVerifier(address verifier) external onlyFactory {
