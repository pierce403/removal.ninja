...
import "./RemovalTaskSimple.sol";
import "./DataBrokerRegistrySimple.sol";

/**
 * @title RemovalTaskFactorySimple
 * @dev Simplified factory contract for creating and managing RemovalTaskSimple instances
 * @notice Creates bounty/escrow tasks for data broker removals
 * @author Pierce
 */
contract RemovalTaskFactorySimple is AccessControl, Pausable, ReentrancyGuard {
    ...
    function createTask(
        ...
        DataBrokerRegistrySimple registry = DataBrokerRegistrySimple(dataBrokerRegistry);
    ...
    function selfAssignToTask(uint256 taskId) external onlyRegisteredWorker validTaskId(taskId) {
        RemovalTaskSimple task = RemovalTaskSimple(tasks[taskId]);
        require(task.currentStatus() == RemovalTaskSimple.Status.Created, "Task not available");
    ...
    function completeTask(uint256 taskId) external validTaskId(taskId) {
        RemovalTaskSimple task = RemovalTaskSimple(tasks[taskId]);
        require(task.currentStatus() == RemovalTaskSimple.Status.Verified, "Task not verified");
        ...
        DataBrokerRegistrySimple(dataBrokerRegistry).recordRemovalCompleted(task.brokerId());
    ...
    function getAvailableTasks() external view returns (uint256[] memory availableTasks) {
        ...
        for (uint256 i = 1; i < nextTaskId; i++) {
            if (tasks[i] != address(0)) {
                RemovalTaskSimple task = RemovalTaskSimple(tasks[i]);
                if (task.currentStatus() == RemovalTaskSimple.Status.Created && 
    ...
    function _assignWorkerToTask(uint256 taskId, address worker) internal {
        ...
        RemovalTaskSimple task = RemovalTaskSimple(tasks[taskId]);
        require(task.currentStatus() == RemovalTaskSimple.Status.Created, "Task not available for assignment");
        require(task.assignedWorker() == address(0), "Task already assigned");
